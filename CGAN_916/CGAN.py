...
def plotSaveTrainLoss(epoch):
    ...
    plt.ylabel("Loss")
    plt.legend()
    # plt.savefig("./images/loss/%d.png" % epoch)
...
for epoch in range(opt.n_epochs):
    for i, (real_data, labels) in enumerate(zip(data_DL, c_DL)):
        ...
        for r in range(1):
            ...
            d_real_loss=F.binary_cross_entropy_with_logits(validity_real,valid)
            #d_fake_loss = adversarial_loss(validity_fake, fake)
            d_fake_loss=F.binary_cross_entropy_with_logits(validity_fake, fake, reduction='none')
            # Total discriminator loss
            #d_loss = (d_real_loss + d_fake_loss) * 5
            d_loss=(d_real_loss+torch.mean(d_fake_loss))*0.5
            #d_loss = d_real_loss + d_fake_loss
            DLoss.append(d_loss.item())
            L1 = ((real_data - gen_imgs).norm(1, 1)).mean()
            g_loss = F.binary_cross_entropy_with_logits(validity_fake,valid, reduction='none')
            gamma = get_gradient_ratios(g_loss, d_fake_loss, validity_fake)
            grad_d_factor = 1.0 / (1.0 - gamma)
            loss_pack_fake = d_fake_loss - g_loss
            scaled_loss_pack_fake = loss_pack_fake * grad_d_factor
            loss_pack = d_real_loss + torch.mean(scaled_loss_pack_fake)
            ...
            #g_loss.backward()
            optimizer_G.step()
            loss_real_val = d_real_loss.item()
            loss_fake_val = torch.mean(d_fake_loss).item()
            loss_d_val = d_loss.item()
            loss_g_val = torch.mean(g_loss).item()
            loss_pack_fake_val = loss_fake_val - loss_g_val
            # print(loss_d_val)
            GLoss.append(loss_g_val)
    print(
        "[Epoch %d/%d] [D loss: %f] [G loss: %f][L1: %f]"
        % (epoch, opt.n_epochs, loss_d_val, loss_g_val,L1.item())
    )
    if (epoch % 100 == 0):
